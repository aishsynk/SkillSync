...
## 2026-09-18 — Demand Details owns content/portfolio-based fit (derived, never eligibility); Mark Skill flows into the team broadcast
- **Decision:** Demand Details (`BatchDetailScreen.kt`) now surfaces, per trainer, a *content fit* computed client-side from the manager's real `api/v2/capability/portfolio` snapshot (the trainer's held course titles + certification names/codes). `PortfolioContentFit` (pure Kotlin, token-set intersection, joined course-code pairing like `PL-300` → `pl300`) answers "we don't hold this exact course, but who has the content?" — a team cleared on PL-300 now surfaces for a custom Power BI demand even though RMS matches names exactly. It is explicitly labelled a derivative ("Content · … (derived from held skills)"), shown alongside — and never in place of — the authoritative gated allocation (`AllocationCandidatesResponse`), and a high overlap can never claim skill/certification/eligibility. People not yet evaluated but with strong overlap appear in an "ADJACENT SKILLS ON FILE" list; an empty portfolio returns "no overlap", never "unsure".
- **Decision (wiring dead code):** the demand page's team-broadcast was never reachable — `shareTarget`/`showMessagePreview` and `EligibilitySheet` were dead (`shareTarget` was never set). This pass wired: "Draft team message" (the exact RMS broadcast: `BatchShare.build` — Course / Assignment ID / Schedule / Mode / Customer / Pax / TOC + the mark-your-skill live-date clause + certified/mock preference), "Copy message", per-trainer "Message", and "Full eligibility breakdown" (EligibilitySheet). A successful Mark Skill now opens the team message draft directly ("mark → share msg to all").
- **Rationale:** The 2026-09-16 boundary keeps matching off Plan but Demand Details is precisely where the team's total portfolio must be read; the client owns `capability_$email` (same cache MainScreen/TeamTab read, zero extra fetch when synced) and only derived display logic is added — RMS still owns allocation and skill writes. Honesty rules: content fit is derived-from-titles only, never an RMS verdict; the broadcast template is unchanged from the server-composed `batch-message` when available with the identical local fallback.
