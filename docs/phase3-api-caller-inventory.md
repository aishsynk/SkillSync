...
| `feature/training/ui/AllocationViewModel.kt` (direct `RetrofitClient.instance` for `getAllocationCandidates`, `getAlternativeTrainers`, `bulkAssignSkill`) | Same file, via `TrainerRepository` (ViewModel already used `ManagerRepository`, gains two more repository params) | Trainer/candidate (reads) + Trainer-skill-write (bulk assignment) | `core/data/TrainerRepository.kt` | `AllocationViewModel` | `GET api/v2/allocation/candidates`, `GET api/data/alternative-trainers`, `POST api/v2/skills/bulk-assign` | None new — existing `MutableStateFlow` UI state (`gatedCandidates`, `globalSearchData`, `bulkResults`) unchanged | `AllocationViewModelTest.kt` (4 tests total, 3 for these) | Phase 3 increment 6 (this commit) |
...
Next increment: `GrowTeamCard.kt` (Trainer share flow), `ActionsViewModel.kt`,
